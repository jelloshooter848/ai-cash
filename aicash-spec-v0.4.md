...
```
{
  ...
  supply: {                               // signed aggregate snapshot, §8(a)
    ...
    mint_id, baseline_model_class,        // §4.1: INSIDE the signed body, so that
                                          // two snapshots disagreeing on either are
                                          // portable proof; they are repeated here,
                                          // not moved out of the top level
...
| Threat | Status | Verifiable from outside by |
|---|---|---|
| ~~Corrupt/fooled attestor mints against fake work~~ | **Eliminated** — earned issuance deleted permanently (§0.1). The stack no longer contains an unbounded failure. | n/a — the mechanism is deleted |
| Mint double-issues (dilution by issuance) | **Partially mitigated — read the covered class carefully.** Signed monotonic supply counters (§3.6) catch **arithmetic** inconsistency: two conflicting signed snapshots, or a broken `outstanding == issued − burned`, are portable proof of nonconformance. They do **not** catch a mint that increments `cumulative_issued_mc` honestly and issues a fortune against no real expenditure (§7.1 issuance is unbounded by design), and they do **not** catch a mint that inserts entries without moving the counters. For an operator excluding its own never-spent treasury, detection probability is **structurally zero**, not low — nobody requests a proof for a coin only the operator holds (`FEDERATION.md` H1, §0.1). Backing is an unverifiable external fact (§7.1) and L10/§0.1 permanently forbid the attestation machinery that would verify it. What is bought is that an *unbounded, silent* rug becomes a *bounded, rate-limited, publicly attributed* one via issuance-velocity gossip and exposure caps (`BOOTSTRAP.md:222`, `:301`). **"Supply is now trustworthy" is not a conclusion this row supports.** | **Anyone**, for the arithmetic (signed snapshots are portable proof). **Nobody**, for the covered class — an honest-counter rug is invisible from outside |
| Mint redefines the unit (dilution by redefinition) | **Mitigated** — `baseline_model_class` is immutable per `mint_id` (§4.1) and rides inside the signed snapshot body, so two signed snapshots disagreeing on it are portable proof of nonconformance. Noted because the counters above cannot detect this: they are denominated in mc, and a baseline change moves what mc means while leaving every counter and the invariant intact. Archivers already diffing signed snapshots see it with no new code. | **Anyone** holding two signed snapshots for one `mint_id` |
| Double-spend via racing two `/exchange` calls | Prevented by atomic check-and-mark (§3.3). Cannot be best-effort. | **The caller** — one call commits, the other is rejected by the mint |
| Bearer token lost to disk failure | **Unrecoverable by design.** Mitigations: custodial mode (§5.2); secret-sharding pattern (§5.1). | Nobody — the loss is by design (L1/L8) |
| Money lost in a mode transition | Persist-before-send applied to withdrawals (§5.3). | **The client itself**, against its own store plus `/v3/status` |
| Client crash mid-payment | Persist-before-send + batch `/v3/status` recovery. | **The client itself**, against its own store plus `/v3/status` |
| Timeout retry double-processed | `idempotency_key`, honored within the retention window (§3.3, §8). | **The caller** — the replay returns the first result |
| Announced burn change not honored (mint and client disagree on the policy in force) | **Mitigated in the design; test it against the mint.** §7.3's notice window (≥ 7 days, or `max_lock_expiry_ms` if longer) plus `burn_policy_next` in the descriptor (§3.6) mean a burn increase is announced before it binds, and every client derives the same effective policy from the same published pair. The guarantee has a second half the schema does not spell out: the mint MUST charge the announced policy from `effective_at` onward. A mint that publishes `burn_policy_next` and keeps charging `burn_policy` past `effective_at` does not merely mis-price — it disagrees with every conformant client about the burn, and each `/v3/exchange` fails `amount_mismatch` (§3.8), which reads as a client bug and stops payments outright. Announcing a change is cheap; switching at `effective_at` is the part worth verifying. | **Anyone** — both policies are published, and the burn actually charged is checkable against them on any exchange the checker makes for itself (the supply counters are aggregates and cannot attribute a burn to one exchange on a mint with concurrent traffic). Divergence is self-announcing: spends start failing `amount_mismatch` at `effective_at` |
| Token captured in transit | TLS; tokens are passwords. **Deployment on the mint side, shipped on the client side.** L17 keeps TLS out of mint code: the mint's own listener is plain HTTP, bound to loopback, with TLS terminated by the mandatory reverse proxy in front of it (`DEPLOYMENT.md` §4) — so whether an operator actually fronts its mint with TLS is a deployment fact nothing in the protocol can check. The other half is no longer open in the reference build: the reference client verifies the certificate chain **and** hostname against the system trust store with no opt-out, and is tested end to end through a TLS proxy, including that an untrusted certificate fails the call (`impl/aicash/wallet.py`, `impl/tests/test_c07_wallet.py::HttpsClientTest`). The residual risk is arbitrary conformant clients: §3 mandates no transport, so **a conformant mint and a conformant client can still hand a bearer token to a man in the middle, and nothing in §3 detects it.** | **The client**, and only if it verifies the certificate — the reference client does, unconditionally. No party can check from the wire that some *other* client did |
| Lock claimed/refunded across the expiry boundary | Deterministic single-path evaluation at commit time; client grace convention; published `grace_ms`/precision (§3.4). | **Either party**, from the mint's published `grace_ms`/`mint_time` and the commit outcome |
| Payee seizes unearned channel remainder | Impossible — payee never learns unrevealed chain links (§9.1). | Structural — nothing to check: the payee never holds the witness |
| Payer claws back drawn value before expiry | Impossible — claim path requires the payee-generated output secret the payer never holds (§3.4, §9.1). | Structural — nothing to check: the payer never holds the payee's output secret |
| Payee fails to settle before channel expiry | Drawn-but-unsettled value refunds to the payer at `T`; disclosed, bounded by the payee's own settlement discipline (§9.1). | **The payer**, at expiry |
| Payee vanishes with prefund | Payer auto-refunds at expiry; loss bounded to the refund call's burn (§9.1, §7.3). | **The payer**, at expiry |
| Swap counterparty claims silently and refunds its own leg | Closed — the mint MUST disclose the claim witness of a spent locked output via `/v3/status` (§3.5, §11). | **Anyone** — the witness is disclosed by `/v3/status` |
| Payer funds escrow locks to a self-invented preimage | Closed — arbiter attestation to both parties + mandatory payee funding verification (§9.3). | **The payee**, before funding — the verification is mandatory |
| Untagged channel chain leaks witnesses via public lock hashes | Closed — mandatory domain-separated chain derivation (§9.1). Implementations must not omit the tag. | **Nobody outside the implementation** — a client-side derivation convention the mint cannot see or enforce |
| Double-spend against serve-then-batch-redeem | Quantified exposure ≤ per-client window; redeem-immediately for untrusted clients (§9.2). | **The serving party**, by redeeming; the bound is its own policy, not a mint guarantee |
| Refund-after-delivery in escrow | Named failure; prevented by deadline ordering + evidence-release path; arbiter silence recorded (§9.3). | **The parties**, against the arbiter's signed record |
| Arbiter corruption | Single arbiter: bounded per-milestone, reputation-priced (§10.2). Panel (rung construction): any single defection — early reveal or silence — is bounded to `value/n` and provable against the arbiter's signed vote; full-milestone theft requires `k` colluding arbiters plus the payee (§9.3). | **The parties**, against the arbiter's signed vote |
| Supervised agent exfiltrates allowance to bearer | **Bounded, not closed — and the bound is not this flag.** The no-bearer-withdrawal flag (§6.1(4)) shuts one exit: while it is set, that agent's §5.3 withdrawals fail. A gate on one exit bounds nothing by itself, because it does not stop the account *spending*. §6.1(4) names §5.3 and only §5.3, so read literally it leaves every other way value moves out of a supervised perimeter — a custodial transfer to an agent under a different operator, or a pull grant pointed at one, which is the same transfer with the payee pulling the trigger — and whoever receives it withdraws to bearer normally, one hop later. **What actually bounds total possible outflow is the cap schedule (§6.1(2)) and the freeze (§6.1(3))**: those apply to every debit, pulls and the burn component of a withdrawal included (§7.3), which is why §6.1 states its guarantee as a bound on outflow rather than as a record of payments. The flag's job is to stop that bound being *evaded* by converting an allowance into unfreezable bearer tokens, and it does that job only if every route out of the flagged agent's operator perimeter is gated with it — a conformance obligation §6.1(4)'s scope guard implies and its sentence does not spell out. **"The flag closes it" is not a conclusion this row supports: a flagged agent with no caps is an unbounded agent.** | **The operator only** — caps, freeze state and §6.1(7) statements. No outside party can see a fleet's exposure, and no party at all can see an exit the mint chose not to gate |
| Pull authorization drains a frozen/capped account | Closed — pulls count against caps and are suspended by freeze (§6.1(2,3,6)). | **The operator**, from the enumerated pull errors and the statement |
| Anonymous tier hollowed by rate-limit gerrymander | **Published, not enforced — this is a disclosure rule, not a protocol guarantee.** §3.7 makes intra-bearer non-discrimination a conformance MUST and requires both tiers in `limits` (§3.6), so a mint that *publishes* a discriminatory anonymous tier is machine-detectable and routing is the defense. That is the whole of it. Nothing compares the published tiers to what is served: a mint publishing equal tiers and throttling anonymous callers harder is invisible to this mechanism, and rate-limit **enforcement is a documented stub** in the reference build (L17 — everyone is served as the anonymous tier, which is trivially non-discriminatory) with real enforcement living in proxy configuration the protocol cannot read. A mint may additionally throttle its entire bearer mode uniformly while serving custodial traffic richly; that narrowing is conformant and visible in the published limits. **"Anonymous callers are served no worse" is a claim about a mint operator's proxy config, not something this row can deliver.** | **Anyone** can read the two published tiers. **Nobody** can verify what is actually enforced — enforcement is proxy configuration |
| Mint-visible correlation (plain mints) | Cash-level privacy only; disclosed (§6.2). Privacy Profile closes it for mints that opt in. **Caveat: the profile's blind-signature wire protocol is a documented stub** — not implemented in the v0.4 reference build (L17: descriptor advertisement and obligations only), so for any mint built on it the second sentence is a specification, not a deployed mitigation. | Nobody — a disclosed property of plain mints, not a defect |
| Subpoena of historical exchange graph | Bounded by the retention compact (§8); Privacy Profile mints prune by mandate with a finite lock window. **Caveat: retention is a promise about the mint's own storage, unobservable from outside, and the Privacy Profile half rests on the same documented stub as the row above** (L17). What a plain mint keeps is what it says it keeps. | **Nobody outside the mint** — retention is a promise about the mint's own storage |
| Cross-mint rate exploitation / free option | Accepted as arbitrage; disclosed; mitigations conventional (§11). | **The parties** — it is arbitrage, disclosed, not a defect |
| Sybil registration | Anchor identity at the operator/billing level; per-agent uniqueness is nearly meaningless. | **The operator**, at the billing/identity level |

**Reading the third column.** It names who, standing outside the mint, can check that the mitigation is actually in force. It is here because roughly a third of these rows are client-side conventions or operator configuration a mint cannot enforce and no counterparty can observe — a status of "closed" earned inside the protocol reads, at a glance, exactly like one that depends on somebody else's proxy config or on a stub the reference build never implemented. **"Structural" and "Anyone" are the only entries that survive a hostile counterparty;** the rest are assumptions, and are listed as such on purpose.
